...
<head>
    ...
    <script src="dwr/interface/RetailServiceBean.js"></script>
                                                    ...
                                                    <tbody>
                                                         <c:forEach var="ret" items="${list}">
                                                            <tr>
                                                                ...
                                                                <td class="text-center">
                                                                	<button data-id="${ret.id_retail}" class="btn btn-primary btn-sm deleteButton" type="button" title="Eliminar" data-toggle="tooltip" data-placement="bottom">
			                                                            <i class="fa fa-trash-o"></i>
			                                                        </button>
                                                                </td>
                                                                <td class="text-center">
                                                                	<button data-id="${ret.id_retail}" class="btn btn-warning btn-sm transButton" type="button" title="Transferir clientes" data-toggle="tooltip" data-placement="bottom">
    ...
    $(document).ready( function() {
        ...
        $('.deleteButton').click( function() {
            ...
            */
        	...
        	swal({   
    			title: "Un momento!",   
    			text: 'Espere por favor, verificando plaza <br><br><div class="progress" style="height:20px;"><div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 100%;">Cargando...</div></div>',   
    			html: true, 
    			showConfirmButton: false 
    		});
            
            console.log("Id Retail", $btn.data('id'))
            
            RetailServiceBean.getTotalStore($btn.data('id'), function(data){
            	console.log("Total de tiendas", data)
            	if (data > 0) {            		
            		swal('Alerta', 'Esta plaza cuenta con clientes', 'warning')
            	} else {
            		swal({
            			title: 'Eliminar plaza',
            			text: '¿Deseá continuar?',
            			showCancelButton: true,
            			cancelButtonText: 'No',
            			confirmButtonText: 'Si'
            		}, function() {
            			location.href = "cfgretaildel.htm?ids="+ $btn.data('id');
            		})
            	}
            })
        ...
        $('.transButton').on('click', function(){
        	var $btn = $(this)
        	swal({
    			title: 'Transferir clientes a otra plaza',
    			text: '¿Deseá continuar?',
    			showCancelButton: true,
    			cancelButtonText: 'No',
    			confirmButtonText: 'Si'
    			...
    			swal({   
        			title: "Un momento!",   
        			text: 'Espere por favor, obteniendo plaza <br><br><div class="progress" style="height:20px;"><div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 100%;">Cargando...</div></div>',   
        			html: true, 
        			showConfirmButton: false 
        		});
    			
    			RetailServiceBean.getRetailByCriteria({id_supplier: "${useraccess.id_supplier}"}, function(data) {
    				console.log("Data", data)
    				if (data && data.length > 0) {
    					var opt = ""
    					data.forEach(function(e, i) {
    						if (e.id_retail != $btn.data('id')){
	    						opt += "<option value='" + e.id_retail + "'>" + e.name + "</option>"
    						}
    						
    						swal({   
    		        			title: "Seleccionar plaza",   
    		        			text: 'Seleccione la plaza a la que se transferirán los clientes<br><br><div class="row"><div class="col-xs-12"><div class="form-group"><label for="input" class="col-sm-2 control-label">Plazas:</label><div class="col-sm-10"><select name="status" id="inputStatus" class="form-control" required="required">' + opt + '</select></div></div></div>',   
    		        			html: true, 
    		        			showCancelButton: true,
    		        			cancelButtonText: 'Cancelar',
    		        			confirmButtonText: 'Continuar'
    		        		}, function() {
    		        			var idrto = $('#inputStatus').val()
    		        			swal({   
    		            			title: "Un momento!",   
    		            			text: 'Espere por favor, moviendo clientes <br><br><div class="progress" style="height:20px;"><div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 100%;">Cargando...</div></div>',   
    		            			html: true, 
    		            			showConfirmButton: false 
    		            		});
    		        			RetailServiceBean.changeStoresRetail({
    		        				id_retail: $btn.data('id'),
    		        				id_retail_to: idrto
    		        			}, function(data) {
    		        				if (data) {
    		        					swal('Exito', 'Se cambiaron las tiendas con exito', 'success')
    		        				} else {
    		        					swal('Error', 'No se cambiaron las tiendas, intente más tarde', 'danger')    		        					
    		        				}
    		        			})
    		        		});
    					})
    				} else {
    					swal('Alerta', "Plazas no obtenidas, intente más tarde", 'warning')
    				}
    			})
    		})
        })
