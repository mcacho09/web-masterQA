...
/**
 * Controller para la vista para modificar productos
 * @author jorge
 * @since 18-12-2014
 ...
 */
public class ProductViewUpdCtrl extends SimpleFormController {
	...
	protected Object formBackingObject(HttpServletRequest request) throws ServletException, ProductNotFoundException {
		logger.debug("---- FORMBACKINGOBJECT ----");
		/*-------------------------------------------------------*/
		// Se obtiene los datos del producto
		Long id_product = ServletRequestUtils.getLongParameter(request, "id");
		Product record = financialService.getProductById(id_product);
		/*-------------------------------------------------------*/
		// Se setea command con los datos del objeto de dominio
		UpdProductDTO command = new UpdProductDTO();
		command.setActive(record.getActive());
		command.setFlag(record.getFlag());
		...
		command.setId_product(record.getId_product());
		command.setLogin(record.getLogin());
		command.setModified(record.getModified());
		command.setName_long(record.getName_long());
		command.setName_short(record.getName_short());
		command.setOrderby(record.getOrderby());
		command.setId_brand(record.getId_brand());
		command.setPrice_cost(record.getPrice_cost());
		command.setPrice_sale(record.getPrice_sale());
		command.setImage(record.getImage());
		command.setPrice_cost_box(record.getPrice_cost_box());
		command.setType(record.getType());
		command.setPiece_in_box(record.getPiece_in_box());
		command.setPrice_sale_box(record.getPrice_sale_box());
		command.setTax(record.getTax());
		
		return command;
	}

	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws ServletException, ProductNotFoundException, UserNotFoundException, AccessNotFoundException {
		...
		String flag = ServletRequestUtils.getStringParameter(request, "flag", NO_ACTIVE);
		
		// Se setea y se persiste el objeto para actualizar el objeto de dominio Product
		UpdProductDTO dto = (UpdProductDTO) command;
		dto.setActive(active);
		...
		dtn.setMessage("Producto <span class='label label-success'>"+dto.getName_short()+"</span> actualizado");
		dtn.setPriority("1");
