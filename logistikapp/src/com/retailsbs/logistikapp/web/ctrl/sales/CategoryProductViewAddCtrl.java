...
/**
 * Controller para la vista para agregar categorias de productos
 * @author jorge
 * @since 11-12-2014
 ...
 */
public class CategoryProductViewAddCtrl extends SimpleFormController {
	...
	public CategoryProductViewAddCtrl() {
		setSessionForm(true);
	...
	protected Object formBackingObject(HttpServletRequest request) throws ServletException {
		...
		AddCategoryProductDTO command = new AddCategoryProductDTO();
		return command;
	}
	...
	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws ServletException, UserNotFoundException, AccessNotFoundException {
		...
		// Se setea y se persiste el objeto para crear el objeto de dominio CategoryProduct
		AddCategoryProductDTO dto = (AddCategoryProductDTO) command;
		dto.setActive(active);
		dto.setCreated(new Date());
		dto.setOrderby(orderby);
		dto.setLogin(useracegi.getUserlogin());
		// Se persiste el objeto
		Long id = financialService.addCategoryProduct(dto);
		...
		dtn.setMessage("Nueva categor&iacute;a producto <span class='label label-primary'>"+dto.getName()+"</span> creada");
		dtn.setPriority("1");
