...
public class CountryViewUpdCtrl extends SimpleFormController {
	...
	protected Map<String, Object> referenceData(HttpServletRequest arg0, Object arg1, Errors arg2) throws Exception{
		...
		model.put("useracces", useraccess);
	...
	protected Object formBackingObject(HttpServletRequest request) throws ServletException, CountryNotFoundException{
		logger.debug("---- FORMBACKINGOBJECT ----");
		// Se obtiene objeto de dominio del pais
		Long id = ServletRequestUtils.getLongParameter(request, "id");
		Country record = retailService.getCountryById(id);
		/*-------------------------------------------------------*/
		// Se setea command con los datos del objeto de dominio
		UpdCountryDTO command = new UpdCountryDTO();
		...
		command.setPrefix(record.getCode());
		return command;
	}

	protected ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors) throws ServletException, CountryNotFoundException{
		...
		// Se setea objeto para modificar country
		UpdCountryDTO dto = (UpdCountryDTO) command;
		...
		// para notificar que se modifica el country 
		...
		dtn.setMessage("Pa&iacute;s <span class='label label-success'>"+dto.getName()+"</span> actualizado");
		dtn.setPriority("1");
